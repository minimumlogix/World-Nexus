...
export const isPlaceholder = (v) => v && typeof v === "object" && FUNCTION_PLACEHOLDER in v;
...
export const getTransferables = (v, result = []) => {
    if (v instanceof ArrayBuffer) {
        ...
        for (const item of v)
            getTransferables(item, result);
    }
    else if (v && typeof v === "object") {
        for (const key in v)
            getTransferables(v[key], result);
    }
    return result;
};
export const createRegistry = () => {
    const callbacks = new Map();
    const callbackProxies = new Map();
